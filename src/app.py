...
from video_io import pick_video_dialog, load_video, save_frame, compute_homography_from_4points, warp_points
...
def on_canvas_click(event):
    ...
    # Rectify mode (4 clicks)
    if S.rectify_click_mode:
        if len(S.rectify_click_points) < 4:
            S.rectify_click_points.append((x_img, y_img))
            app.draw()
            if len(S.rectify_click_points) == 4:
                try:
                    H, H_inv, dst = compute_homography_from_4points(S.rectify_click_points, dst_size=S.rectify_dst_size)
                    S.homography = H
                    S.homography_inv = H_inv
                    S.rectify_dst_size = dst
                    S.rectify_click_mode = False
                    S.rectify_click_points = []
                    messagebox.showinfo("Rectificado", "Homografía calculada correctamente. Ahora los cálculos se hacen en vista rectificada.")
                except Exception as e:
                    messagebox.showerror("Error", f"No se pudo calcular la homografía: {e}")
                    S.homography = None
    ...
    if S.ref_line is not None and S.def_offset is not None and S.att_offset is not None:
        # Si hay homografía: trabajamos en vista rectificada para la detección de proximidad
        if S.homography is not None:
            # ref_line en rectified
            ref0_rect, ref1_rect = warp_points([S.ref_line[0], S.ref_line[1]], S.homography)
            theta = geometry.line_angle(tuple(ref0_rect), tuple(ref1_rect))
            normal = geometry.perpendicular_unit_vector(theta)
            origin_rect = np.array(ref0_rect, dtype=float)
            ref_dir = geometry.unit_vector_from_angle(theta)
            L = 2000

            # centros en rectified
            def_center_rect = origin_rect + normal * S.def_offset
            att_center_rect = origin_rect + normal * S.att_offset

            # extremos largos en rectified
            def_p0_rect = def_center_rect - ref_dir * L
            def_p1_rect = def_center_rect + ref_dir * L
            att_p0_rect = att_center_rect - ref_dir * L
            att_p1_rect = att_center_rect + ref_dir * L

            # transformar puntos a imagen para calcular distancia visual
            def_p0_img = tuple(warp_points([tuple(def_p0_rect)], S.homography_inv)[0].astype(int))
            def_p1_img = tuple(warp_points([tuple(def_p1_rect)], S.homography_inv)[0].astype(int))
            att_p0_img = tuple(warp_points([tuple(att_p0_rect)], S.homography_inv)[0].astype(int))
            att_p1_img = tuple(warp_points([tuple(att_p1_rect)], S.homography_inv)[0].astype(int))

            d_def = geometry.point_line_distance((x_img,y_img), def_p0_img, def_p1_img)
            d_att = geometry.point_line_distance((x_img,y_img), att_p0_img, att_p1_img)

            if d_def <= PROX_DIST and d_def <= d_att:
                S.mouse_drag['dragging'] = True
                S.mouse_drag['which'] = 'def'
                # guardamos proyección del ratón en coordenadas rectificadas
                mouse_rect = warp_points([(x_img, y_img)], S.homography)[0]
                mouse_proj = float(np.dot(np.array([mouse_rect[0]-origin_rect[0], mouse_rect[1]-origin_rect[1]]), normal))
            ...
            # comportamiento original (imagen sin rectificar)
...
def on_canvas_move(event):
    ...
    if S.mouse_drag['dragging']:
        if S.mouse_drag['which'] == 'ref':
            ...
            # Si tenemos homografía, hacemos los cálculos en espacio rectificado
            if S.homography is not None:
                # transformamos ref_line y ratón a rectified
                ref0_rect, ref1_rect = warp_points([S.ref_line[0], S.ref_line[1]], S.homography)
                theta = geometry.line_angle(tuple(ref0_rect), tuple(ref1_rect))
                normal = geometry.perpendicular_unit_vector(theta)
                origin_rect = np.array(ref0_rect, dtype=float)
                ...
                mouse_rect = warp_points([(x_img, y_img)], S.homography)[0]
                mouse_proj_now = float(np.dot(np.array([mouse_rect[0]-origin_rect[0], mouse_rect[1]-origin_rect[1]]), normal))
                ...
                if start_proj is None or start_off is None:
                    # sin valores previos, asignamos directamente
                ...
                # update crosshairs in image coords
                if S.mouse_drag['which'] == 'def':
                    S.cross_def = (x_img,y_img)
                else:
                    S.cross_att = (x_img,y_img)
            else:
                # comportamiento original en espacio imagen
                theta = geometry.line_angle(S.ref_line[0], S.ref_line[1])
                normal = geometry.perpendicular_unit_vector(theta)
                origin = np.array(S.ref_line[0], dtype=float)
...
class App:
    def __init__(self, root):
        ...
        # 2. Rectificar
        self.b_rect = tk.Button(left_buttons, text="Rectificar (H)", command=self.start_rectify_mode)
        self.b_rect.pack(side=tk.LEFT, padx=4, pady=4)
    ...
    def start_rectify_mode(self):
        S.rectify_click_mode = True
        S.rectify_click_points = []
        print("RECTIFY MODE ON: haz 4 clics (TL, TR, BR, BL) para definir un rectángulo del campo")
